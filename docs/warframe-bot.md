...
例如：`裂缝 帮助`、`蹲裂缝 帮助`、`查价 帮助`、`权限 帮助`。

- 帮助内容含：能力说明、语法模板、简短案例。
- `蹲裂缝` 这类主题别名只展示对应章节（不是完整「蹲」帮助）。
- 参数解析失败时会标明「未能解析」片段，并给出可能参数与同一套模板/案例。

新命令必须注册 `CommandHelpSpec`，并在解析失败时走 `formatParseFailure`。
...
- 裂缝、钢铁裂缝、九重天裂缝
- 夜灵 / 地球黑夜 / 三傻：同一条订阅，黑夜开始前 3 分钟提醒。不支持夜灵白天或夜灵竞速
- 仲裁：节点开始前 3 分钟提醒
- 地球 / 希图斯、金星 / 福尔图娜、火卫二、扎里曼、圣所 / 科维兽、1999 赏金，可指定档位和任务类型
- 双衍王境：目标情绪开始前 3 分钟提醒
- 入侵
...
普通群员的新订阅和续订后剩余有效期最多为 7 天。群主、管理员、机器人所有者（`1071342037`）及持有「长期 / 管理」角色的成员可以创建永久有效的订阅。

多数查询类命令（世界状态、赏金、掉落、遗物、周常、榜单等）也已接入同一套 `命令 帮助`；未单独写进文档的别名可直接试「别名 帮助」。

#### 群角色权限（可扩展）

| 角色 | 命令名 | 能力 |
| --- | --- | --- |
| 推送 | `push` | 开关群推送（总开关 / 分类） |
| 长期 | `premium` | 长期订阅、长期蹲价 |
| 管理 | `manager` | 上述全部 + 禁言/解除禁言；兼容旧「推送授权」 |

内置身份不入库：机器人所有者、本群群主/管理员默认拥有全部能力，并可授予/取消角色。
...
以后新增可控权限时：先加 capability 常量，再挂到已有角色或新增角色；业务命令统一 `hasCapability(...)`，一般不必改命令语法。
...
同词条紫卡查价的 2~4 个词条除完整词条名外支持社区简写：单字简写（`多`、`基`、`弹`、`暴伤` / `爆伤`），组合简写 `双暴` / `双爆`（= 暴击率 + 暴击伤害）后面可再接任意词条简写与阵营字母，粘连写作 `双爆多`、`双爆基`、`双爆g`，也可以分开写 `双爆 基`。负词条用 `-X` / `负X`，末位 `-任意` / `负任意` 表示必须有负但不限具体负词条。`zk` / `紫卡查价` 与 `蹲紫卡` 使用同一套解析。

WM 交易市场支持低价监控：

| 功能 | 命令 / 别名 | 示例 |
| --- | --- | --- |
| 普通物品低价监控 | `market-watch-item` / `蹲低价` / `蹲价` | `蹲低价 膛室 Prime 50p 7天` |
| 满级物品低价监控 | 同上 | `蹲低价 满级 川流不息 120p` |
| 紫卡词条订单监控 | `market-watch-riven` / `蹲紫卡` | `蹲紫卡 托里德 暴击率 暴击伤害 多重射击 -弹药上限 500p` |
| WM 蹲价列表 | `market-watch-list` / `我的蹲价` | `我的蹲价` |
| WM 蹲价续期 | `market-watch-renew` / `续蹲价` / `续蹲` | `续蹲价 低价 1 3天` |
| 删除普通物品监控 | `market-watch-remove` / `不蹲价` | `不蹲价 1` |
| 删除紫卡监控 | 同上 / `不蹲紫卡` | `不蹲紫卡 1` |

普通物品查价卡按价格分别展示在线最低 7 条和离线最低 3 条，并标出“游戏中 / 在线 / 离线”状态；卡片后的快捷私聊文案只使用游戏中或在线卖家。普通物品低价监控仍只匹配 WM 上可见且卖家处于游戏中的卖单；紫卡只匹配公开直售且正负词条集合完全一致的订单。命中后按订单或价格变化去重，同群相同条件会合并提醒并 @ 所有订阅者。未指定有效期时默认 3 天，普通成员有效期最多 7 天，群主、管理员、机器人所有者和持有「长期/管理」角色的成员可使用长期监控。`我的蹲价` 会同时展示续期示例。

WM 普通物品、紫卡武器和低价监控在名称无法精确匹配时，会返回 1~3 个可能的中文或英文名称，按提示重新输入即可；掉落等物品查询也使用同一套候选提示。
...
- `榜单 赋能 满级 前20`
- `榜单 MOD 前10`
- `榜单 紫卡` / `榜单 紫卡 前10`（实验室各武器最低价，条数默认 20、范围 10–50）
- `榜单 未开紫卡`
- `榜单 战甲` / `榜单 战甲套装`（一套）
- `榜单 武器` / `榜单 武器套装`（一套）
- `榜单 成品武器`
...
`武器`、`战甲` 默认查一套；只有写成 `武器部件`、`战甲部件` 才查散件。`榜单 紫卡` 使用实验室热门武器价排行（`min_price`），可用 `前N`（10–50，实验室接口硬顶）；结果缓存 1 小时。
...
| 仲裁表 | `arbitration` / `arbi` / `仲裁` / `仲裁表`；列出全部节点，含当前整点 |
| 高价值仲裁 | `arbitration-high` / `高价值仲裁`；仅 A- 及以上 |
| 入侵 | `invasion` / `入侵` |
| 警报 | `alert` / `警报` |
| 每日突击 | `sortie` / `突击` / `每日突击` |
...
| 深井 / 夜帽 | `bounty-deepmines` / `深井赏金` / `深矿赏金` / `夜帽` / `蘑菇` |

合一众任务出现在地球、金星赏金卡片上，不单独查询。隔离库出现在火卫二赏金卡片上。利润收割者没有赏金任务列表。深井三种任务常驻，奖励来自公开导出掉落表；钢铁之路每 2.5 小时在这三种里轮换，和金星赏金同一刷新点，但官方世界状态不写当前是哪一种。
...
| 沉沦之地 | `descendia` / `爬塔` / `沉沦之地` | 本周 21 层挑战与奖励；默认普通+钢铁两张图；可加 `普通`/`钢铁`/`文本` |
...
| 沉沦之地 | 周一 00:00 UTC / 08:00 北京时间 | 不纳入自动周报（首版仅独立命令） |
...
| 中英术语翻译 | `translate` / `翻译` / `术语` | `翻译 Critical Chance`、`翻译 暴击几率` |
| 查询续页 | `result-more` / `更多` / `下一页` / `下页` | `更多`、`更多 2` |
...
掉落来源、整件装备的部件来源，以及遗物反查结果过多时会分页展示。卡片底部会提示页码和剩余条数；在同一会话中由同一用户发送 `更多` 即可继续，`更多 2` 可跳到第 2 页。也可在原查询末尾追加 `第N页`，例如 `掉落 内融核心 第2页`。续页上下文仅在内存保留 5 分钟，按平台、会话和用户隔离；重新查询会替换自己的旧上下文，机器人重启或超时后请重新查询。

物品名称输入错误时，机器人会提示若干可能的正确名称；该提示适用于掉落、WM 查价、紫卡查价和蹲价等涉及物品名称的查询。未开紫卡支持类型简写，例如 `查价 近战未开`、`查价 步枪未开`、`查价 曲翼未开`、`查价 同伴未开`（也可用手枪/霰弹/组合枪/zaw）。`翻译`（别名 `术语`）可输入中文或英文 Warframe 术语，返回对应的中英文名称。
...
| 积分商店 | `积分商店` / `商店` | 查看分类摘要；`称号` / `边框` / `背景` 出带缩略图的商店图 |
| 预览 | `预览 <item>` / `商店预览` | 用签到卡预览称号、边框或背景，不消耗积分 |
| 购买 | `购买 <item> [quantity]` | 购买积分商店商品或外观 |
| 装备 | `装备 <item>` | 穿上已拥有外观，或卸下为默认称号/边框/背景 |
| 背包 | `背包` / `我的道具` | 查看战斗道具和已购外观 |
| 赠送积分 | `赠送 <target> <amount>` / `送积分` | 向今日已签到群友赠送积分 |
| 积分排行 | `积分排行` / `排行榜` | 本群积分排行 |
| 对战排行 | `对战排行` | 本群 MMR 排行 |
| 俄罗斯轮盘 | `射爆` | 每群独立的六发左轮；中弹者禁言 30 秒并重新装填；同群连发或并发按到达顺序各打一枪 |

对战结算中仅发起方消耗对战券并支付完整积分，被挑战方支付一半积分；被挑战方获胜时，积分奖励、胜利奇遇积分和胜利经验均减半。

群聊普通消息中包含“原神”时，娱乐插件会尝试将发言者禁言 30 秒；机器人自身消息、私聊以及信息不完整的会话不会触发。`射爆` 的每群弹仓状态保存在数据库中，并通过群级队列按到达顺序结算连发或并发请求。
...
- `entertainment_cosmetic`
- `entertainment_loadout`
...
| `entertainment_cosmetic` | 已购外观 |
| `entertainment_loadout` | 当前装备的称号、边框、签到背景 |
