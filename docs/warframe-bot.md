...
WM 普通物品、紫卡武器和低价监控在名称无法精确匹配时，会返回最多 5 个可能的中文或英文名称，按提示重新输入即可；掉落等物品查询也使用同一套候选提示。
...
- `榜单 赋能 满级 前20`
- `榜单 MOD 前10`
- `榜单 紫卡` / `榜单 紫卡 前10`（各武器当前直售最低价，条数默认 20、范围 10–200）
- `榜单 未开紫卡`
- `榜单 战甲` / `榜单 战甲套装`（一套）
- `榜单 武器` / `榜单 武器套装`（一套）
- `榜单 成品武器`
...
`武器`、`战甲` 默认查一套；只有写成 `武器部件`、`战甲部件` 才查散件。`榜单 紫卡` 按每把武器当前直售最低价排序，可用 `前N`（10–200）；首次刷新较慢，结果缓存 1 小时。
