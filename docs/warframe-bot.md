...
同词条紫卡查价的 2~4 个词条除完整词条名外支持社区简写：单字简写（`多`、`基`、`弹`、`暴伤`），组合简写 `双暴` / `双爆`（= 暴击率 + 暴击伤害）后面可再接任意词条简写与阵营字母，粘连写作 `双爆多`、`双爆基`、`双爆g`，也可以分开写 `双爆 基`。负词条用 `-X` / `负X`，末位 `-任意` / `负任意` 表示必须有负但不限具体负词条。`zk` / `紫卡查价` 与 `蹲紫卡` 使用同一套解析。
...
普通物品查价卡按价格分别展示在线最低 7 条和离线最低 3 条，并标出“游戏中 / 在线 / 离线”状态；卡片后的快捷私聊文案只使用游戏中或在线卖家。普通物品低价监控仍只匹配 WM 上可见且卖家处于游戏中的卖单；紫卡只匹配公开直售且正负词条集合完全一致的订单。命中后按订单或价格变化去重，同群相同条件会合并提醒并 @ 所有订阅者。未指定有效期时默认 3 天，普通成员有效期最多 7 天，群主、管理员和推送授权成员可使用长期监控。`我的蹲价` 会同时展示续期示例。
...
- `榜单 赋能 满级 前20`
- `榜单 MOD 前10`
- `榜单 紫卡` / `榜单 紫卡 前10`（实验室各武器最低价，条数默认 20、范围 10–200）
- `榜单 未开紫卡`
- `榜单 战甲` / `榜单 战甲套装`（一套）
- `榜单 武器` / `榜单 武器套装`（一套）
- `榜单 成品武器`
...
`武器`、`战甲` 默认查一套；只有写成 `武器部件`、`战甲部件` 才查散件。`榜单 紫卡` 使用实验室热门武器价排行（`min_price`），可用 `前N`（10–200）；结果缓存 1 小时。
...
物品名称输入错误时，机器人会提示若干可能的正确名称；该提示适用于掉落、WM 查价、紫卡查价和蹲价等涉及物品名称的查询。`翻译`（别名 `术语`）可输入中文或英文 Warframe 术语，返回对应的中英文名称。
