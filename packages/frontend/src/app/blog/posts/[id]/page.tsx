	...
	return (
		<div className="relative">
			...
			<article className="max-w-3xl mx-auto">
				...
				>
					...
					<div className="flex items-center gap-3 text-xs text-muted-foreground mb-4">
						<Link
							...
							className="font-semibold text-primary hover:text-primary/70 uppercase tracking-wider"
						>
					...
					{/* Summary */}
					{article.summary && (
						<p className="text-lg text-muted-foreground leading-relaxed border-l-2 border-primary/30 pl-4 italic">{article.summary}</p>
					)}
						...
						<div className="flex flex-wrap gap-2 mt-4">
							{article.tags.map((tag: Tag) => (
								<Link key={tag.id} href={`/blog/tags/${tag.slug}`}>
									<Badge variant="outline" className="border-primary/30 hover:bg-primary/5">
										{tag.name}
									</Badge>
								</Link>
							))}
				...
				{/* Content */}
				<MotionDiv initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.4, delay: 0.2 }}>
