...
export default function AdminGalleryCategoriesPage() {
  ...
  const load = () => getAdminGalleryCategories().then(setCategories);
  useEffect(() => { load(); }, []);

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (editingId) {
      await updateGalleryCategory(editingId, { name, slug });
      toast.success('已更新');
      setEditingId(null);
    } else {
      await createGalleryCategory({ name, slug });
      toast.success('已添加');
    }
    setName(''); setSlug('');
    load();
  ...
  const handleDelete = async (id: string) => {
    if (!confirm('确定删除这个分类？分类下的图片会变成未分类。')) return;
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold mb-6">图库分类管理</h1>
          ...
          <TableBody>
            {categories.map((cat: AnyRecord) => (
              <TableRow key={cat.id}>
                ...
                <TableCell>{cat._count?.images || 0}</TableCell>
                <TableCell>
